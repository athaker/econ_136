...
class ElectionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.features[idx, :].astype(np.float32)), torch.tensor(self.labels[idx].astype(np.float32))

# Function to compute percentage change
def compute_td_pct(djw, index, days):
    ntd = djw.truncate(after=index).iloc[-1]["Closing Value"]
    if days > 0:
        pct = (djw[index:index+timedelta(days=1)].iloc[-1]["Closing Value"] - ntd) / djw[index:index+timedelta(days=days)].iloc[-1]["Closing Value"]
    else:
        pct = (ntd - djw[index+timedelta(days=days):index].iloc[0]["Closing Value"]) / ntd
...
if __name__ == "__main__":
    ...
    features = ["prev_held_office_democratic","prev_held_office_republican","previous_party_1","previous_party_2","previous_party_3"]
